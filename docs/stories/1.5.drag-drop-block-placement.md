...
- [x] Task 1: Drag Gesture Implementation (AC: 1)
  ...
  - [x] Test drag responsiveness on actual devices
...
- [x] Task 5: Block Placement Animation (AC: 5)
  ...
  - [x] Test placement animations across device types
...
- Rounded grid projection uses the live touch point to anchor blocks, so overlapping an occupied cell now keeps the new piece mobile instead of dragging the placed block.
- Added a regression test covering collision previews to guard against future regressions.
